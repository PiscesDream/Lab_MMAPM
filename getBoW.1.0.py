...
#        x.extend(data[tag]['x'])
    ...
    tagging = lambda x: x.split('/')[-1].split('_')[0] 
    tagset = list(set([tagging(tag) for tag in data]))
    for tag in data:
        # differs in every datasets
        data[tag]['y'] = tagset.index(tagging(tag))
        print tag, data[tag]['y']
    tagset = list(tagset)
    print 'tagset:', tagset
...
def main():
    getWithCodebook(K=200, T=1.0)#, groups=100)
